...
public class SelectCustomer : MonoBehaviour
{
    ...
    void Start()
    {
        CustomerManager = GameObject.Find("Managers").GetComponent<CustomerManager>();
    ...
    void Update()
    {
        ...
        if(GameManager._IsStart && !CustomerManager.HasFinishFirst)
        {
            ...
            if(CustomerNum == -1)CustomerNum = CustomerManager.CurrentCustomers.Length - 1;
            else if(CustomerNum == CustomerManager.CurrentCustomers.Length)CustomerNum = 0;
            gameObject.transform.position = new Vector3 (CustomerManager.CurrentCustomers[CustomerNum].transform.position.x, 3.9f, 10);
        }
